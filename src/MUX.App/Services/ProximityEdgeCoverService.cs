...
/// <summary>
/// Stable per-window black edge covers with generous proximity dragging. A user does not need to
/// hit the thin black wall precisely: the entire near-edge interaction strip can start a drag.
/// </summary>
public sealed class ProximityEdgeCoverService : IDisposable
{
    ...
    public ProximityEdgeCoverService()
    {
    ...
    private sealed class CoverSession : IDisposable
    {
        ...
        private readonly EdgeWindow _top;
        private readonly EdgeWindow _right;
        private readonly EdgeWindow _bottom;
        private readonly EdgeWindow _left;
        ...
        public CoverSession(IntPtr target)
        {
            ...
            _top = Create(EdgeSide.Top);
            _right = Create(EdgeSide.Right);
            _bottom = Create(EdgeSide.Bottom);
            _left = Create(EdgeSide.Left);
        }

        private EdgeWindow Create(EdgeSide side)
        {
        ...
        private void SetThickness(EdgeSide side, int value)
        {
            ...
            var next = Math.Clamp(value, _minimumThickness, max);
    ...
    private sealed class EdgeWindow : Window, IDisposable
    {
        ...
        private static readonly TimeSpan HandleAnimation = TimeSpan.FromMilliseconds(100);
        ...
        public EdgeWindow(EdgeSide side, Func<int> getThickness, Action<int> setThickness)
        {
            _side = side;
        ...
        private static Border BuildHandle(EdgeSide side, ScaleTransform scale)
        {
            ...
            for (var i = 0; i < 3; i++)
            {
                ...
                {
                    Width = side is EdgeSide.Top or EdgeSide.Bottom ? 18 : 1.4,
                    Height = side is EdgeSide.Top or EdgeSide.Bottom ? 1.4 : 18,
                    RadiusX = 0.7,
                    RadiusY = 0.7,
                    Margin = side is EdgeSide.Top or EdgeSide.Bottom
                        ? new Thickness(0, i == 0 ? 0 : 2.4, 0, 0)
                        : new Thickness(i == 0 ? 0 : 2.4, 0, 0, 0),
            ...
            return new Border
            {
                Background = new SolidColorBrush(Color.FromArgb(249, 23, 23, 26)),
                BorderBrush = new SolidColorBrush(Color.FromRgb(100, 100, 109)),
        ...
        public void Update(NativeRect targetRect, int thickness, uint dpi, NativePoint? cursor)
        {
            ...
            // At 100% scaling this is a 48px grab zone inward from the adjustable boundary.
            _proximity = ScaleForDpi(48, _dpi);
            _overlayRect = OverlayRect(targetRect, _thickness, _proximity, _side);
        ...
        {
            var scale = _dpi / 96.0;
            var width = Math.Max(1.0, _overlayRect.Width / scale);
            var height = Math.Max(1.0, _overlayRect.Height / scale);
            var thickness = Math.Max(1.0, _thickness / scale);
            var proximity = Math.Max(0.0, _proximity / scale);
            ...
            {
                _cover.Width = width;
                _cover.Height = Math.Min(height, thickness);
                Canvas.SetLeft(_cover, 0);
                Canvas.SetTop(
                    _cover,
                    _side == EdgeSide.Top
                        ? 0
                        : Math.Min(proximity, Math.Max(0, height - _cover.Height)));
            }
            else
            {
                _cover.Width = Math.Min(width, thickness);
                ...
                Canvas.SetLeft(
                    ...
                    _side == EdgeSide.Left
                        ? 0
                        : Math.Min(proximity, Math.Max(0, width - _cover.Width)));
            }
        }
        ...
        private void DrawHandle(NativePoint? cursor)
        {
            ...
            var hot = _dragging || (along && distance <= _proximity);
            SetHot(hot);
            if (!hot)
            {
                return;
            ...
            var scale = _dpi / 96.0;
            var longPx = ScaleForDpi(58, _dpi);
            var shortPx = ScaleForDpi(22, _dpi);
        ...
        private void SetHot(bool hot)
        {
            ...
            {
                EasingMode = hot ? EasingMode.EaseOut : EasingMode.EaseIn
            };

            _handle.BeginAnimation(
                OpacityProperty,
                new DoubleAnimation(_handle.Opacity, hot ? 1.0 : 0.0, HandleAnimation)
                {
            ...
            _handleScale.BeginAnimation(
                ...
                new DoubleAnimation(_handleScale.ScaleX, hot ? 1.0 : 0.82, HandleAnimation)
                {
                    EasingFunction = easing
                },
                HandoffBehavior.SnapshotAndReplace);
            _handleScale.BeginAnimation(
                ...
                new DoubleAnimation(_handleScale.ScaleY, hot ? 1.0 : 0.82, HandleAnimation)
                {
                    EasingFunction = easing
                },
                HandoffBehavior.SnapshotAndReplace);
        ...
        private bool IsInteractivePoint(NativePoint point)
        {
            ...
            var boundary = InnerBoundary();
            return _side is EdgeSide.Top or EdgeSide.Bottom
                ? Math.Abs(point.Y - boundary) <= _proximity || IsOnBlackCover(point)
                : Math.Abs(point.X - boundary) <= _proximity || IsOnBlackCover(point);
        }
        ...
        private bool IsOnBlackCover(NativePoint point)
        {
            return _side switch
            {
                EdgeSide.Top => point.Y >= _targetRect.Top && point.Y < _targetRect.Top + _thickness,
                EdgeSide.Bottom => point.Y >= _targetRect.Bottom - _thickness && point.Y < _targetRect.Bottom,
                EdgeSide.Left => point.X >= _targetRect.Left && point.X < _targetRect.Left + _thickness,
                EdgeSide.Right => point.X >= _targetRect.Right - _thickness && point.X < _targetRect.Right,
                _ => false
            };
        }
        ...
        private static NativeRect OverlayRect(
            NativeRect target,
            int thickness,
            int proximity,
            EdgeSide side)
        {
            return side switch
            {
                EdgeSide.Top => new NativeRect(
                    ...
                    target.Top,
                    target.Right,
                    Math.Min(target.Bottom, target.Top + thickness + proximity)),
                EdgeSide.Bottom => new NativeRect(
                    target.Left,
                    Math.Max(target.Top, target.Bottom - thickness - proximity),
                    target.Right,
                    target.Bottom),
                EdgeSide.Left => new NativeRect(
                    target.Left,
                    target.Top,
                    Math.Min(target.Right, target.Left + thickness + proximity),
                    target.Bottom),
                EdgeSide.Right => new NativeRect(
                    Math.Max(target.Left, target.Right - thickness - proximity),
                    target.Top,
                    target.Right,
                    target.Bottom),
                _ => target
